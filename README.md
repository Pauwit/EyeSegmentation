...
```
├── states/
│   └── eye_6.pth         # Trained model weights
...
4.  The training loop will save the best model weights (based on validation Dice score) to the path specified by the `path` argument in the `train_uresnet` function (e.g., `states/eye_6.pth`).
5.  Training progress, including loss and Dice scores for both training and validation sets, will be plotted at the end.

### 3. Running Inference

1.  Open the `detector.ipynb` notebook.
2.  Set the `MODEL_PATH` variable to the location of your trained model file (e.g., `"states/eye_6.pth"`).
3.  Set the `IMAGE_PATH` variable to the path of the M0 image you want to process.
4.  (Optional) Set the `SAVE_PATH` variable to a file path if you wish to save the resulting mask as a PNG image. Set it to `None` to disable saving.
5.  Run all the cells in the notebook. A plot will be displayed showing the original image and the predicted mask side-by-side.
